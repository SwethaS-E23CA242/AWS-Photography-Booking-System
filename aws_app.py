...
def admin_dashboard():
    ...
    try:
        ...
        photographer_users_response = photographer_users_table.scan()
        bookings_response = bookings_table.scan()

        users = users_response.get('Items', [])
        photographers = photographers_response.get('Items', [])
        photographer_users = photographer_users_response.get('Items', [])
        ...
        photographer_user_count = len(photographer_users)
...
def admin_view_users():
    ...
    try:
        ...
        print(f"Error getting users (DynamoDB): {e}")
    # Fallback to local users
...
def admin_reports():
    ...
    try:
        ...
        total_revenue = sum(float(get_photographer_by_id(b.get('photographer_id')).get('rate', 0)) for b in bookings if b.get('status') == 'Completed')
        ...
        print(f"Error getting reports (DynamoDB): {e}")
        bookings = list(bookings_db.values())
        total_revenue = sum(float(get_photographer_by_id(b.get('photographer_id')).get('rate', 0)) for b in bookings if b.get('status') == 'Completed')
        completed_bookings = sum(1 for b in bookings if b.get('status') == 'Completed')
        cancelled_bookings = sum(1 for b in bookings if b.get('status') == 'Cancelled')
    ...
    return render_template('admin_reports.html', report_stats=report_stats, bookings=bookings)
